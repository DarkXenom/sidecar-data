...
(function () {
  ...
  // --- Chip normalization (Phase-A + forward-compat) ---
  function normChip(key, p){
    if(!p) return null;
    const d=(p.detail||'').trim(); if(!d) return null;
    ...
    if(key==='bulk_heat'){                                   // "Bulk deals N (30d)"
    ...
    // Advanced chips (when you re-enable later)
  ...
  // --- Badges (optional, shown if present) ---
